...
(function (factory) {
    ...
    $h = {
        ...
        getDecimalPlaces: function (num) {
            ...
            return !m
                ? 0
                : Math.max(0, (m[1] ? m[1].length : 0) - (m[2] ? +m[2] : 0));
        },
        ...
        handler: function ($el, event, callback, skipOff, skipNS) {
            var ev = skipNS
                ? event
                : event.split(" ").join($h.NAMESPACE + " ") + $h.NAMESPACE;
            if (!skipOff) {
                $el.off(ev);
            }
            $el.on(ev, callback);
    ...
    Rating.prototype = {
        ...
        _renderClear: function () {
            ...
            if ($clr.length) {
                $h.addCss($clr, css);
                $clr.attr({ title: self.clearButtonTitle }).html(
                    self.clearButton
                );
                self.$clear = $clr;
                return;
        ...
        _getClearClass: function () {
            ...
            return (
                ...
                " " +
                (self.inactive ? "" : self.clearButtonActiveClass)
            );
        },
        ...
        _initEvents: function () {
            ...
            self.events = {
                ...
                starMouseMove: function (e) {
                    ...
                    self.$element.trigger("rating:hover", [
                        out.val,
                        out.caption,
                        "stars",
                    ]);
        ...
        _listen: function () {
            ...
            $h.handler(
                ...
                $.proxy(events.initTouch, self)
            );
            $h.handler(
                $rating,
                "click touchstart",
                $.proxy(events.starClick, self)
            );
            $h.handler(
                $rating,
                "mousemove",
                $.proxy(events.starMouseMove, self)
            );
            $h.handler(
                $rating,
                "mouseleave",
                $.proxy(events.starMouseLeave, self)
            );
            if (self.showClear && $clear.length) {
                $h.handler(
                    $clear,
                    "click touchstart",
                    $.proxy(events.clearClick, self)
                );
                $h.handler(
                    $clear,
                    "mousemove",
                    $.proxy(events.clearMouseMove, self)
                );
                $h.handler(
                    $clear,
                    "mouseleave",
                    $.proxy(events.clearMouseLeave, self)
                );
            }
            if ($form.length) {
                $h.handler(
        ...
        _getStars: function (type) {
            ...
            for (i = 1; i <= self.stars; i++) {
                stars +=
                    '<span class="star">' + self[type + "Star"] + "</span>";
            }
            return stars + "</span>";
        },
        ...
        calculate: function (pos) {
            var self = this,
                defaultVal = $h.isEmpty(self.$element.val())
                    ? 0
                    : self.$element.val(),
                val = arguments.length
                    ? self.getValueFromPosition(pos)
                    : defaultVal,
    ...
    $.fn.rating.defaults = {
        ...
        language: "en",
        stars: 5,
        filledStar: '<i class="ecicon eci-star fill"></i>',
        emptyStar: '<i class="ecicon eci-star-o"></i>',
        containerClass: "",
        size: "md",
        animate: true,
        displayOnly: false,
