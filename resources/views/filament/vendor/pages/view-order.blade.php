                        ...
                        <table class="w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($record->products as $product)
                                    @php
                                        $sku = null;
                                        $variationData = $product->pivot->variation ? json_decode($product->pivot->variation, true) : null;
                                        $skuImage = $product->image;
                                        if ($variationData && isset($variationData['sku_id'])) {
                                            $sku = \App\Models\Sku::with('attributeValues.attribute')->find($variationData['sku_id']);
                                            if ($sku && $sku->image) {
                                                $skuImage = $sku->image;
                                            }
                                        }
                                        $rowSkuCode = $variationData['sku_code'] ?? $sku->sku ?? $product->sku ?? 'N/A';
                                        $attributes =
                                            $sku?->attributeValues->map(function ($attrValue) {
                                                return ($attrValue->attribute->name ?? 'Attribute') . ': ' . $attrValue->getDisplayName();
                                            })->toArray() ?? [];
                                    @endphp
                                    <tr>
                                        <td class="px-6 py-4">
                                                    ...
                                                    @if (!empty($attributes))
                                                        <div class="mt-1 text-xs text-gray-500 space-x-1">
                                                            @foreach ($attributes as $attribute)
                                                                <span class="bg-gray-100 px-2 py-1 rounded">{{ $attribute }}</span>
                                                            @endforeach
                                                        </div>
                                                    @endif
                                                </div>
                                        ...
                                        <td
                                            ...
                                            {{ Sohoj::price($product->pivot->total_price) }}
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
