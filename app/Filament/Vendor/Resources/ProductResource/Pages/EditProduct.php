...
class EditProduct extends EditRecord
{
    ...
    protected function afterSave(): void
    {
        ...
        // Auto-generate SKUs if product is variable and has attribute values
        if ($this->record->is_variable_product && $this->record->attributeValues()->count() > 0) {
            $service = new SkuGenerationService();
            $service->generateSkus($this->record);
        }

        // Refresh the form with the latest record data
        $this->record->refresh();
